...
some of the ingredients needed. Snippets with this kind of extra information are
reported to [increase click-throughs by up to 30%](YKKK). 

Rich snippets [started in 2009](http://googlewebmastercentral.blogspot.com/2009/05/introducing-rich-snippets.html). 
...
partners have [agreed to support RDFa Lite](YKK), which is similar to Microdata, 
when used with the Schema.org vocabularies.

So currently the main reason to be using Microdata with Schema.org is that it is
...
appear in the future, the [main uses cases](YKK Hixie on use cases) are currently
